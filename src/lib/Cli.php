		...
		try {
			...
			if ( ! empty( $group_ids ) ) {
				...
				foreach ( $group_ids as $group_id => $model ) {
					...
					if ( count( $model ) === 1 ) {
						$product    = reset( $model );
						$categories = ($dbProduct && self::$cf->is_not_up_cat) ? [] : parent::getProductCategories($product, $oasisCategories);
						...
						$categories   = ($dbProduct && self::$cf->is_not_up_cat) ? [] : parent::getProductCategories( $firstProduct, $oasisCategories);
